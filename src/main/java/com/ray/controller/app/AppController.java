...
//        System.out.println(departs);
		/*
		 * Map<String, Object> map = FastJson.getJson().parse(userinfo.getBody(),
		 * Map.class); Record user = new Record().setColumns(map);
		 */
		// 返回结果
...
//            System.out.println("getUserInfo success");
	...
	public void getDepartment() {
		List<RpLineStructure> ws = RpLineStructure.dao
				.find("select * from rp_line_structure where id >= 1 and id <=10");
	...
	public void getWorkshop() {
		...
		Record resp = new Record();
		String sql1 = "select * from rp_line_structure where pid =(select id from rp_line_structure where productionLine='"
				+ selectDep + "' ) ";
		String sql2 = "select * from rp_line_structure where pid = (select id from rp_line_structure  where pid =(select id from rp_line_structure where productionLine='"
				+ selectDep + "' ) ORDER BY id ASC  LIMIT 1) ";
		String sql3 = "select * from rp_shift_leader where pid=(select id from rp_line_structure  where pid =(select id from rp_line_structure where productionLine='"
				+ selectDep + "' ) ORDER BY id ASC  LIMIT 1) ";

		// 若未选择车间 默认选择pid为1的事业部和pid为11的车间
		if ("".equals(selectDep) || selectDep == null) {
			sql1 = "select * from rp_line_structure where pid = 1 ";
	...
	public void getLine() {
        ...
        String[] managerName = new String[userManager.size()];
		String[] managerId = new String[userManager.size()];
		for (int i = 0; i < userManager.size(); i++) {
//			System.out.println(i);
			managerName[i] = userManager.get(i).getNickname();
			managerId[i] = userManager.get(i).getJobNumber();
		};
		resp.set("leanUserlist", managerId);
		resp.set("leanUserlistName", managerName);
		String sql1 = "select * from rp_line_structure where pid in (select id from rp_line_structure where productionLine='"
				+ selectws + "' ) ";
		String sql2 = "select * from rp_shift_leader where pid = (select id from rp_line_structure where productionLine='"
				+ selectws + "')";
...
//			System.out.println(i);
		...
		String sql = "select * from rp_shift_leader where pid = (select id from rp_line_structure where productionLine='"
				+ selectws + "'";
...
// 查询是否为班长，以及最新精益管理员信息
	public void checkIsLeader() {
		...
		System.out.println(user_jobnumber);
		Boolean isShiftLeader = false;
		Boolean isLeanManager = false;
		List<RpShiftLeader> auditorForLeader = RpShiftLeader.dao.find("select * from rp_shift_leader");
		// 如果发起人是班长，返回精益管理员信息
		for (int i = 0; i < auditorForLeader.size(); i++) {
			if (auditorForLeader.get(i).getShiftLeaderJobNo().equals(user_jobnumber)) {
				isShiftLeader = true;
			}
		}
		Record resp = new Record();
...
//		System.out.println(userManager.toArray().toString());
